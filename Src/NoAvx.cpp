                                ...
                                const float scale, const float dx, const float dy)
{
    ...
    float y0            = -(float)height / (2 * scale) * dy + centerY + imageYShift;
    const float x0Begin = -(float)width  / (2 * scale) * dx + centerX + imageXShift;

    for (size_t pixelY = 0; pixelY < height; ++pixelY, y0 += dy / scale)
    {
        float x0 = x0Begin;

        for (size_t pixelX = 0; pixelX < width; ++pixelX, x0 += dx / scale)
        {
            float x = x0;
            float y = y0;
